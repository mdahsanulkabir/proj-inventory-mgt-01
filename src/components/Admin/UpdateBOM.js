...
const UpdateBOM = () => {
    ...
    //destructure the sfgParts for the list in autocomplete (children)
    const sfgParts = sfgBOMs.map(part => ({
    ...
    //destructure the basic raw materials for the list in autocomplete (children)
    ...
    //combine the previous sfg and rm in a single array list 
    // (for the children auto complete)
    const partOptions = rmParts.concat(sfgParts)
    // console.log(partOptions);
    

    //destructure the source categories for list of auto-complete source-category
    ...
    //destructure the sfg categories for the list of sfg category auto compelete
    ...
    // after selecting a SFG for update, we need to pass the definition of
    // the selected SFG to the auto complete components
    const handleSelectedSFGBOM = ( selectedSFGbom_id ) => {
        const selectedSFG = sfgBOMs.find(sfgBOM => sfgBOM._id === selectedSFGbom_id); // purpose: from the database, get the previous data of the selected SFG
        console.log("selected SFG = ",selectedSFG);
        setMySelectedSFG(selectedSFG);            // need to store the state to pull the Mongo-ID of the selected SFG
        setSFGObject_id(selectedSFG.object_id);   // purpose: for auto-complete store the object_id of the selected SFG
        setSFGName(selectedSFG.material_name);    // purpose: for auto-complete store the name of the selected SFG

        // purpose: set the value={} property of the auto-complete of source with the selected SFG source
        setSFGSource({
            _id : selectedSFG.source_category,
            label : sfgSourceCategoryOptions.find(source_ => source_._id === selectedSFG.source_category).label
        });

        // purpose: set the value={} property of the auto-complete of part category with the selected SFG category
        setSFGCategory({
            _id:selectedSFG.sfg_category,
            label: sfgCategoryOptions.find(cat => cat._id === selectedSFG.sfg_category).label
        });

        // set the text field's text for the selected sfg's SAPcode
        setSfgSAPCode(selectedSFG.sap_code);
        // set the text field's text for the selected sfg's SIScode
        setSfgSisCode(selectedSFG.sis_code);



        //destructure the childrens' properties of the selected SFG and define the keys (pre stored) with the code keys
        const elements = selectedSFG.children.map(child => {
            return {
                _id : child.object_id._id, 
                obj_id : child.object_id.object_id, 
        ...
        // set the children state of for the auto-completes (each children will have individual auto-completes)
        setPartsAndSFG(elements);
    }

    const handleSubmit = async () => {
        const updatedChildren = partsAndSFG.map((part, index) =>  ({
            object_id : part._id,
        ...
        //     sap_code: sfgSAPCode,
        //     sis_code : sfgSisCode,
        //     children : updatedChildren    //  Object.values(partsAndSFG)
        // });
            ...
            <Paper elevation = {3}>
                <Grid container>
                    <Grid item xs={8}>
                        ...
                        >
                            ...
                            <Autocomplete 
                                ...
                                value={sfgSource}
                                getOptionLabel = {(sfgSource) => sfgSource ? sfgSource.label : ""}
                            ...
                            <Autocomplete 
                                ...
                                value={sfgCategory}
                                getOptionLabel = {(sfgCategory) => sfgCategory ? sfgCategory.label:""}
                                ...
                                partsAndSFG.map((singlePartAndSFG, index) => (
                                    <Box key={index} sx={{display : 'flex', alignItems: 'center', justifyContent: 'space-between'}}>
                                        <Autocomplete
                                            disablePortal
                                            options={partOptions}
                                            value={singlePartAndSFG}  //! included this line
                                            sx={{ width: 700 }}
                                            // getOptionLabel={(singlePartAndSFG) => {
                                            //     // console.log(partsAndSFG);
                                            //     return (`${singlePartAndSFG.obj_id}  ${singlePartAndSFG.label}`)
                                                
                                            // }} 
                                            getOptionLabel={(value) => value.obj_id +" "+ value.label }
                                            // getOptionLabel={(singlePartAndSFG) => singlePartAndSFG ? singlePartAndSFG.obj_id  : ""} // found no problem with only obj_id

                                            // getOptionLabel={(singlePartAndSFG) => singlePartAndSFG ? singlePartAndSFG.obj_id +" "+ partOptions?.find(lbl => lbl._id === singlePartAndSFG._id).label : ""}
                                            isOptionEqualToValue = {(singlePartAndSFG, value) => {
                                                // console.log("value =",value);
                                                // console.log("singlePartAndSFG =", singlePartAndSFG);
                                                return  singlePartAndSFG._id === value._id
                                            }}
                                            renderOption={(props, option) => (
                                                <Box component="li" sx={{ '& > img': { mr: 2, flexShrink: 0 } }} {...props}>
                                                {/* <img
                    ...
                    <Grid item xs={4}>
                        ...
                        <List sx={{overflow: "auto", height:"40vh"}}>
                            {
                                sfgBOMs?.map(sfgBOM => {
                                    return (
